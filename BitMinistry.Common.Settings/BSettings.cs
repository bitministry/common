...
namespace BitMinistry.Common.Settings
{
    ...
    public class BSettings
    {
        ...
        ///</summary>
        public static void Init ( string connectionName = "infra" , string[] excludeModules = null )
        {
            ...
            if (All == null && ! IgnoreDatabase )
            {
                ...
                if (Config.ConnectionStrings.ContainsKey(connectionName))
                ...
                using (var sql = new BSqlRawCommander(_connectionName ))
                    if (sql.ExecuteScalar("select OBJECT_ID('Setting')") != DBNull.Value)
                        InitHashSet();
        ...
        private static void InitHashSet()
        {
            ...
            {
                All = sql.GetDataRows($"select * from {typeof(Setting).Name} where {where}")
        ...
        {
            using (var sql = new BSqlRawCommander( _connectionName ))
                All[name] = sql.GetDataRows($"select * from {typeof(Setting).Name} where Name ='{name}'")
